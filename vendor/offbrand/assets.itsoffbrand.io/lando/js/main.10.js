...
function initLetterHoverBodyChange() {
  const blackElements = document.querySelectorAll('[data-gl-hover="#292C20"]');
  ...
  const outlineButtons = document.querySelectorAll(".btn-w.outline");
  const limeButtons = document.querySelectorAll(".btn-w.lime");

  document
    .querySelectorAll('[data-gl-hover="#111718"], [data-gl-hover="#d2ff00"]')
    .forEach((el) => (el.style.pointerEvents = "auto"));

  // Store initial colors for resetting
  let isHovering = false;

  const handleHover = (color, buttonChanges) => {
    isHovering = true;
    document.body.style.color = color;
    landoGroup.style.color = color;
    norrisGroup.style.color = color;

    // Apply button changes based on which element is hovered
    if (buttonChanges === "black") {
      // When black element is hovered - outline buttons get dark green text with lime background and lime border
      outlineButtons.forEach((btn) => {
        btn.style.color = "var(--color--dark-green)";
        btn.style.backgroundColor = "var(--color--lime)";
        btn.style.borderColor = "var(--color--lime)";
      });
    } else if (buttonChanges === "lime") {
      // When lime element is hovered - lime buttons get dark green color and border
      limeButtons.forEach((btn) => {
        btn.style.color = "var(--color--dark-green)";
        btn.style.borderColor = "var(--color--dark-green)";
      });
    }
  };
  ...
  const resetColors = () => {
    isHovering = false;
    document.body.style.color = "var(--color--dark-green)";
    landoGroup.style.color = "var(--color--dark-green)";
    ...
    limeButtons.forEach((btn) => {
      btn.style.color = ""; // Reset to CSS value
      btn.style.borderColor = "var(--color--lime)";
    });
  };
...
function initButtons() {
  // Select all buttons with class .btn-w inside .holding-ln4-pos
  const btns = document.querySelectorAll(".holding-ln4-pos .btn-w");
  ...
  // Animate buttons to visible state with delay
  gsap.to(btns, {
    autoAlpha: 1,
    y: "0rem",
    duration: 0.6,
...
function initWelcomeMessage() {
  const baseStyle =
    "background-color: #FDFDF5; color: black; font: 400 1em monospace; padding: 0.5em 0;";
  const boldStyle =
  ...
  console.log(
    "%c built by %cOFF+BRAND. %c > https://itsoffbrand.com",
    baseStyle, // Styles " powered by "
    boldStyle, // Styles "OFF+BRAND."
    "" // Resets to default console styling for " > https://itsoffbrand.com"
  );
}
